  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {unread > 0 && (
          <span className="absolute right-2 top-2 size-2 rounded-full bg-brand-600 ring-2 ring-card dark:bg-brand-400" aria-hidden />
        )}
      </Button>
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <ul className="max-h-80 divide-y divide-border overflow-y-auto">
              {notifications.map((notification) => (
                <li key={notification.id} className={cn('px-4 py-3', notification.unread && 'bg-accent/40')}>
            ...
            >
              View all activity <ArrowUpRight className="size-3.5" aria-hidden />
            </Link>
          </motion.div>
        )}
      </AnimatePresence>
